...
| PET 패키지 | `PET/` | PET 폴더에 접근하여 requirements.txt 실행하여 패키지 설치
...
- **STEP 3**: STEP 2에서 검출된 영역만 남기고 나머지(배경 등)는 투명 처리 →
...
하드코딩되어 있으므로, STEP 1에서 만든 파일을 PET 폴더로 복사해야 함
